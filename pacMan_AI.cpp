	...
	struct DeadEnd_Value
	{
		...
		DeadEnd_Value& operator () (GameField &gameField,int myID)
		{
			...
			for( int i = 0; i < 4; i++)
			{
				...
				{
					if( GetDisBetween( gameField.players[i].row, gameField.players[i].col,gameField.players[myID].row, gameField.players[myID].col) - 2 *savesteps[gameField.players[myID].row][gameField.players[myID].col] <= 0)
						i_will_die = true;
				}
			}
