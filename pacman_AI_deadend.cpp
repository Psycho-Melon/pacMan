...
namespace Value {
	...
	*/
	...
	struct DeadEnd_Value
	{
		...
		DeadEnd_Value& operator () (GameField &gameField,int myID)
		{
			...
			for( int i = 0; i < 4; i++)
			{
				...
				{
					if( GetDisBetween( gameField.players[i].row, gameField.players[i].col,gameField.players[myID].row, gameField.players[myID].col) - 2 *save_steps[gameField.players[myID].row][gameField.players[myID].col] <= 0)
						if( save_steps[gameField.players[i].row][ gameField.players[i].col] < save_steps[gameField.players[myID].row][ gameField.players[myID].col]) i_will_die = true;
				}
			}
	...
	/*int DeadEnd_Value(GameField &gameField,int myID)//y表示第一个下标，x表示第二个下标，表示自己将要走到的那个胡同格子
	{
		...
		if( (depth - CHAR_START) )
		{
			...
			while(Flag)
			{
				...
				else if(x < (Width - 1) && ( save_steps[y][x+1] - save_steps[y][x]) == 1 && !(gameField.fieldStatic[y][x] & wallEast)) x++;
				else if(y < (Height -1) && ( save_steps[y+1][x] - save_steps[y][x]) == 1 && !(gameField.fieldStatic[y][x] & wallSouth)) y++;
				else Flag = 0;
			...
			{//开始搜索
				...
				int dx = 0,dy = 0,rivalcount = 0, smallest_value = 100;
					...
					int flag = 0;
					for(rivalAct = Pacman::stay; rivalAct < 4; rivalAct++)
					{
						if (gameField.ActionValid(enemy, rivalAct))
						{
							...
							Value += DeadEnd_Value( y,x,myID,gameField.players[myID].strength);	//递归
	...
	int dead_end_depth(GameField &gameField,int myID,int y, int x)
	{
		for( int i = 0; i < Height; i++)
		{
			for( int j = 0; j < Width; j++)
		...
		int y = gameField.players[myID].row,x = gameField.players[myID].col,counter = 0;
		int Height = gameField.height, Width = gameField.width;
		while( save_steps[y][x] - CHAR_START)
		{
			counter++;
			if     (x > 0 && ( save_steps[y][x-1] - save_steps[y][x]) == 1 && !(gameField.fieldStatic[y][x] & wallWest)) x--;
			else if(y > 0 && ( save_steps[y-1][x] - save_steps[y][x]) == 1 && !(gameField.fieldStatic[y][x] & wallNorth)) y--;
